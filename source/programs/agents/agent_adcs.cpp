...
// agent requests
int32_t request_control_mode(char *request, char* response, void *cdata);
...
char request[1000], result[REQBUFSIZE];
...
int main(int argc, char *argv[])
{
                                     ...
                                     MAXBUFFERSIZE)))
    {
        ...
        sprintf(buffer_string,"Exiting: Could not start server for [%s:adcs]\n",node_name.c_str());
        log_string = string(buffer_string);
        cout << log_string << endl;
        log_agent(log_string);
        exit (AGENT_ERROR_JSON_CREATE);
    }

    log_agent("Server is setup");
    ...
    if ((iretn=agent_add_request(cdata,"control_mode",request_control_mode)))
        exit (iretn);
...
//    rvector torque;
    ...
    while(agent_running(cdata)) {
        ...
        if( ((currentmjd() - utc_start_gps)*86400) > 600){
            ...
            if (0){
                if (iscbeat.utc != 0) {
                    iretn = agent_send_request(cdata, iscbeat, "pdu_switch 10 900", response, 300, 2.);
                }
            } else {
        ...
        if( ((currentmjd() - utc_start_st)*86400) > 600){
            ...
            if (0){
                if (iscbeat.utc != 0) {
                    iretn = agent_send_request(cdata, iscbeat, "pdu_switch 4 900", response, 300, 2.);
                }
            }
            else {
        ...
        if( ((currentmjd() - utc_start_tcu)*86400 > 600) && (control_mode != CONTROL_MODE_OFF)){
            ...
            if (0){
                if (iscbeat.utc != 0) {
                    iretn = agent_send_request(cdata, iscbeat, "pdu_switch 3 900", response, 300, 2.);
                }
            } else {
...
//    char buffer_imu[255] = {'\0'};
    ...
    FILE *fp;
    string cnodedir = get_cnodedir(cdata[0].node.name);
    ...
    while (agent_running(cdata)){
        ...
        if (cdata->devspec.stt[0]->flag & DEVICE_FLAG_ACTIVE)
        {
            ...
            if (dt_att > ts_att ||  dt_sttomega > ts_sttomega){
                ...
                //                {
                //                    if ((pbeat.utc || mbeat.utc) && cdata[1].devspec.stt[0]->utc != 0.)
                //                    {
                //                        newstt.utc = cdata[1].devspec.stt[0]->utc;
                //                        newstt.utc = currentmjd(0.);;
                //                        newstt.omega = cdata[1].devspec.stt[0]->omega;
                //                        newstt.att = cdata[1].devspec.stt[0]->att;
                //                        newstt.temp = cdata[1].devspec.stt[0]->temp;
                //                        newstt.volt = cdata[1].devspec.stt[0]->volt;
        ...
        if (cdata->devspec.imu[0]->flag & DEVICE_FLAG_ACTIVE)
        {
            ...
            if (dt_mag > ts_mag ||  dt_imuomega > ts_imuomega)
            {
                ...
                //                {
                //                    if (pbeat.utc && cdata[1].devspec.imu[0]->utc != 0.)
                //                    {
                //                        newimu.utc = cdata[1].devspec.imu[0]->utc;
                //                        newimu.omega = cdata[1].devspec.imu[0]->omega;
                //                        newimu.mag = cdata[1].devspec.imu[0]->mag;
                //                        newimu.accel = cdata[1].devspec.imu[0]->accel;
                //                        newimu.temp = cdata[1].devspec.imu[0]->temp;
                //                        newvalue = true;
                //                    }
                //                }
                //                else
...
int32_t turn_on_st(){
    ...
    if(0){
        if (iscbeat.utc != 0) {
            iretn = agent_send_request(cdata, iscbeat, "pdu_switch 4 900", isc_response, 300, 2.);
        }
    }
...
int32_t turn_on_gps(){
    ...
    if(0){
        if (iscbeat.utc != 0) {
            iretn = agent_send_request(cdata, iscbeat, "pdu_switch 10 900", isc_response, 300, 2.);
        }
    }
...
int get_motion()
{
    ...
    agent_send_request(cdata, mbeat,request,result,REQBUFSIZE,5);
    json_parse(result,&cdata[1]);
...
int set_simulated()
{
    ...
    agent_send_request(cdata, pbeat,request,result,REQBUFSIZE,5);
...
int get_physics()
{
    ...
    cdata[1].devspec.gps[0]->utc = cdata[1].devspec.imu[0]->utc = cdata[1].devspec.stt[0]->utc = 0;
    if (pbeat.utc)
    {
        memset((void *)result,0,REQBUFSIZE);
        sprintf(request,"getvalue {"
                ...
                "\"device_stt_att_000\","
                "\"device_stt_omega_000\"}");
        iretn = agent_send_request(cdata, pbeat,request,result,REQBUFSIZE,5);
        if (iretn > 0)
        {
            json_parse(result,&cdata[1]);
            cdata[0].node.utcoffset = cdata[1].node.utcoffset;
        }
    }

    if (iretn < 0)
    {
        cdata[1].devspec.gps[0]->utc = cdata[1].devspec.imu[0]->utc = cdata[1].devspec.stt[0]->utc = currentmjd(0.);
        cdata[1].devspec.gps[0]->position = GEOC_HONOLULU;
        cdata[1].devspec.gps[0]->velocity = rv_zero();
    }
...
int32_t request_control_mode(char *request, char *response, void *cdata)
{
    ...
    double value[4];
    sscanf(request,"%*s %s %lf %lf %lf %lf",response,&value[0],&value[1],&value[2],&value[3]);
